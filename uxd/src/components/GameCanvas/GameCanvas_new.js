...
export default {
    ...
    data() {
        return {
            bot: this.positionData["turtlebot"],
            bot_image: Image,
            heroes: [this.positionData["captain_america_0"], this.positionData["iron_man"], this.positionData["hulk"]],
            heroes_reached: [false, false, false],
            heroes_names: ["Captain_America", "Iron_Man", "Hulk"],
            heroes_images: Image,
            obstacles: [this.positionData["obstacles_0"], this.positionData["obstacles_1"], this.positionData["obstacles_2"], this.positionData["obstacles_3"]],
    ...
    mounted() {
        ...
        this.$refs.board.addEventListener('click', function (event) {
            ...
            that.heroes.forEach(function (hero, i) {
                let newCoord = that.calcNewCoord(hero[4], 20);
                if (x >= newCoord[0] && x <= newCoord[0] + newCoord[2] && y >= newCoord[1] && y <= newCoord[1] + newCoord[3]) {
    ...
    methods: {
        ...
        drawObstacles() {
            this.obstacles.forEach(obstacle => {
                let newCoord = this.calcNewCoord(obstacle[4], 20);

                this.boardContext.beginPath();
                this.boardContext.rect(
                    this.cellSize + newCoord[0],
                    this.cellSize + newCoord[1],
                    this.cellSize + newCoord[2],
                    this.cellSize + newCoord[3]
                );
        ...
        drawHeroes() {
            this.heroes.forEach(function (hero, i) {
                ...
                */
                ...
                let newCoord = this.calcNewCoord(hero[4], 20);
                ...
                this.boardContext.drawImage(this.heroes_images[i], x, y, width, height);*/
                this.boardContext.drawImage(this.heroes_images[i], newCoord[0], newCoord[1], newCoord[2], newCoord[3]);

            }, this);
        },
        checkHeroCollision() {
            let newCoordBot = this.calcNewCoord(this.bot[4], 20);
            this.heroes.forEach(function (hero, i) {
                let newCoordHero = this.calcNewCoord(hero[4], 20);
                if (this.heroes_reached[this.order.indexOf(this.heroes_names[i])]) return;
                if (newCoordBot[0] + newCoordBot[2] >= newCoordHero[0] && newCoordBot[0] <= newCoordHero[0] + newCoordHero[2] && newCoordBot[1] + newCoordBot[3] >= newCoordHero[1] && newCoordBot[1] <= newCoordHero[1] + newCoordHero[3]) {
        ...
        drawBot() {
            ...
            let newCoord = this.calcNewCoord(this.bot[4], 20);
            this.boardContext.drawImage(this.bot_image, newCoord[0], newCoord[1], newCoord[2], newCoord[3]);
        ...
        checkCoinCollision() {
            ...
            this.coins.forEach(coin => {
                ...
                if (newCoord[0] + newCoord[2] >= coin[0] && newCoord[0] <= coin[0] + this.coinSize
                    && newCoord[1] + newCoord[3] >= coin[1] && newCoord[1] <= coin[1] + this.coinSize) {
                    coin[2] = true;
        ...
        calcNewCoord(center, diameter){
            let x = center[0] - diameter/2;
            let y = center[1] - diameter/2;
            let width = diameter;
            let height = diameter;
